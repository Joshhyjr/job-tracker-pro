    ...
    <div className="space-y-4">
      ...
      <Sheet open={Boolean(selectedApplication)} onOpenChange={(open) => { if (!open) setSelectedApplication(null); }}>
        <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-xl">
          {selectedApplication && <><SheetHeader className="border-b pb-4 pr-8"><div className="flex items-center gap-3">{/* The drawer header is a company display surface, so it shares the central logo component. */}<CompanyLogo companyName={selectedApplication.companyName} jobLink={selectedApplication.jobLink} companyDomain={selectedApplication.companyDomain} logoUrl={selectedApplication.companyLogoUrl} size="md" /><div className="min-w-0"><SheetTitle>{selectedApplication.jobTitle}</SheetTitle><SheetDescription>{selectedApplication.companyName} · {selectedApplication.location}</SheetDescription></div></div></SheetHeader><div className="space-y-5 py-5"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border px-2 py-1 text-xs font-semibold" style={getResponseStatusBadgeStyle(currentResponseStatus(selectedApplication))}>{currentResponseStatus(selectedApplication)}</span><span className="text-xs text-muted-foreground">Applied {formatDisplayDate(selectedApplication.dateApplied)}</span></div><dl className="grid gap-3 rounded-md border p-4 text-xs sm:grid-cols-2">{[
